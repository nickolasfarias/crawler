...
class Api::V1::QuotesController < ApplicationController
  ...
  def show
    ...
    if Tag.all.any? { |tag| tag.name == params[:tag] } && Quote.all.any? { |quote| quote.tags[0] == params[:tag] }
    ...
    elsif Tag.all.any? { |tag| tag.name == params[:tag] } && Quote.all.none? { |quote| quote.tags[0] == params[:tag] }
      quotes = []
      authors = []
      authors_abouts = []
      tag_divs = []
